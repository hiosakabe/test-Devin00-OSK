...
class FamilyStatusSequencePredictor(nn.Module):
    """
    A sequence prediction model that incorporates user status information including family structure.
    This model extends the status sequence model by adding family information as additional input features.
    """
...
# Generate random training data with status information including family structure
def generate_family_status_sequences(num_sequences=3000, min_length=3, max_length=10, 
                                    blank_prob=0.1, multi_input_prob=0.15):
    """
    Generate random sequences with associated user status information including family structure.
    Each sequence is paired with random status attributes (age, gender, family structure).
    """
    ...
    # Define possible family structures
    family_structures = [
        "single",           # 独身
    ...
    # Family structure to one-hot encoding mapping
    family_to_idx = {family: i for i, family in enumerate(family_structures)}
    
    for _ in range(num_sequences):
        # Randomly choose sequence length
        length = random.randint(min_length, max_length)
        
        # Generate a sequence (similar to status_sequence_model.py)
    ...
    return sequences, status_info, family_structures
...
def train_family_sequence_model(num_sequences=3000, num_epochs=100, batch_size=64):
    """Train a sequence prediction model that incorporates family status information."""
    print("Generating training data...")
    sequences, status_info, family_structures = generate_family_status_sequences(num_sequences)
    ...
    for i, seq in enumerate(sequences):
        for j in range(1, len(seq)):
            ...
            status_train.append(prepare_family_status(status_info[i], family_structures))
    ...
    model = FamilyStatusSequencePredictor(vocab_size, embedding_dim, hidden_dim, status_dim)
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.savefig('family_training_loss.png')
    print("Training loss plot saved to family_training_loss.png")
    
    # Save the trained model
    torch.save(model.state_dict(), "family_sequence_model.pth")
    print("Model saved to family_sequence_model.pth")
    ...
    with open('family_training_data.pkl', 'wb') as f:
        pickle.dump(data, f)
    
    print("Training data saved to family_training_data.pkl")
...
def run_family_inference_tests(model, family_structures, element_to_idx, idx_to_element):
    """Run inference tests with different status combinations including family structure."""
...
if __name__ == "__main__":
    # Train the model
    model, sequences, status_info, family_structures, element_to_idx, idx_to_element = train_family_sequence_model(
        num_sequences=3000,
        num_epochs=100,
        batch_size=64
    )
    ...
    # Run inference tests
    run_family_inference_tests(model, family_structures, element_to_idx, idx_to_element)
